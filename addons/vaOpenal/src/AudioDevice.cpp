...
void preloadSources()
{
	...
	{
	    std::cout << "sources = " << sourceIndex << std::endl;
		alGenSources(1, &sourceID);
        int err=0;
        if((err=alGetError()) != AL_FALSE) {
            std::cout <<"Couldn't generate source name, maxmimum numbers of sources: " << sourceIndex << std::endl;
            break;
        }
...
//		std::cout << "OpenAL extensions: " << alGetString(AL_EXTENSIONS) << std::endl;
	...
	if (alcIsExtensionPresent (NULL, (const ALCchar *) "ALC_ENUMERATION_EXT") == AL_TRUE)
	{
		...
		while (*s != '\0')
		{
			std::cout << "OpenAL available device: " << s << std::endl;
			while (*s++ != '\0');
		}
	}
	else
	{
		std::cout << "OpenAL device enumeration isn't available." << std::endl;
	}

	// Print default device name
	std::cout << "OpenAL default device: "
	...
	if (myDevice)
	{
		std::cout << "OpenAL current device: "
